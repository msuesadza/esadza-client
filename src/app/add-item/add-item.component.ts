...
export class AddItemComponent implements OnInit {
  ...
  onSubmit():void{
    ...
    {
      console.log(formData.get('file'));
      this.url="http://localhost:9999/addNewItem";
    }
    else
    {
      this.url="http://localhost:9999/addNewItemUrl";
    }
  ...
  checkAvailability() {
    this.http.post<boolean>("http://localhost:9999/checkItemId",this.newFoodItems.id).subscribe(
      res=>{
        this.present=res;
      },err=>{
